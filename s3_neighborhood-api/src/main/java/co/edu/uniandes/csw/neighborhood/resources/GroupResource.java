...
public class GroupResource {
    ...
    public GroupDetailDTO createGroup(GroupDetailDTO group, @PathParam("neighborhoodId") Long neighId) throws BusinessLogicException {
        ...
        GroupDetailDTO groupDTO = new GroupDetailDTO(groupLogic.createGroup(group.toEntity(), neighId));
    ...
    public List<GroupDetailDTO> getGroups(@PathParam("neighborhoodId") Long neighId) {
        ...
        List<GroupDetailDTO> groups = listEntity2DTO(groupLogic.getGroups(neighId));
    ...
    @GET
    @Path("{groupsId: \\d+}")
    public GroupDetailDTO getGroup(@PathParam("groupsId") Long groupsId, @PathParam("neighborhoodId") Long neighId) {
        LOGGER.log(Level.INFO, "Looking for  group from resource: input: {0}", groupsId);
        GroupEntity groupEntity = groupLogic.getGroup(groupsId, neighId);
        if (groupEntity == null) {
    ...
    @PUT
    @Path("{groupsId: \\d+}")
    public GroupDetailDTO updateAuthor(@PathParam("groupsId") Long groupsId, @PathParam("neighborhoodId") Long neighId, GroupDetailDTO group) throws BusinessLogicException {
    ...
    @DELETE
    @Path("{groupsId: \\d+}")
    public void deleteGroup(@PathParam("groupsId") Long groupsId, @PathParam("neighborhoodId") Long neighId) {
        LOGGER.log(Level.INFO, "Deleting group from resource: input: {0}", groupsId);
        if (groupLogic.getGroup(groupsId, neighId) == null) {
            throw new WebApplicationException("Resource /groups/" + groupsId + " does not exist.", 404);
        }
        groupLogic.deleteGroup(groupsId, neighId);
        LOGGER.info("Group deleted from resource: output: void");
    }

    /**
     * Converts an entity list to a DTO list for groups.
     *
     * @param entityList Group entity list to be converted.
