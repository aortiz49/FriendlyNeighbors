...
public class ResidentProfileResource {
    ...
    @Inject
    private ResidentLoginLogic residentLoginLoginLogic;

    /**
     * Creates a new resident with the information received in request, then an
     * identical object is returned with id generated by DB
     *
    ...
    public ResidentProfileDetailDTO createResident(ResidentProfileDetailDTO resident, @PathParam("neighborhoodId") Long neighId) throws BusinessLogicException {
        LOGGER.log(Level.INFO, "Creating resident");

        ResidentLoginDTO loginDTO = resident.getLogin();

        if (loginDTO == null) {
            throw new WebApplicationException("Login resource does not exist.", 404);
        }
     ...
     * request body.
     *
     * @param residentsId ID from resident to be updated. Must be a sequence of
     * digits.
     * @param neighId parent neighborhood
    ...
    /**
     *
     * Connects /residents route with /postsToView route which are dependent of
     * resident resource, by redirecting to the service managing the URL segment
     * in charge of the viewers
     *
     * @param residentsId id from resident from which the resource is being
     * accessed
     * @param neighId parent neighborhood
    ...
    /**
     *
     * Connects /residents route with /attendedEvents route which are dependent
     * of resident resource, by redirecting to the service managing the URL
     * segment in charge of the members
     *
     * @param residentsId id from resident from which the resource is being
     * accessed
     * @param neighId parent neighborhood
    ...
    /**
     * Connects /residents path with /businesses path which depends on the
     * resident resource, by redirecting to the service managing the URL segment
     * in charge of the businesses
     *
     * @param residentsId id from resident from which the resource is being
     * accessed
     * @param neighId parent neighborhood
    ...
    /**
     * Connects /residents path with /events path which depends on the resident
    ...
    /**
     * Connects /residents path with /notifications path which depends on the
     * resident resource, by redirecting to the service managing the URL segment
    ...
    @POST
    @Path("{residentsId: \\d+}/album")
    public ResidentProfileDetailDTO associatePictureToResident(@PathParam("residentsId") Long postsId, @PathParam("neighborhoodId") Long neighId, String pic) throws BusinessLogicException, WebApplicationException {
        LOGGER.log(Level.INFO, "Associating album to resident from resource: input: postsId {0}", new Object[]{postsId});
        if (residentLogic.getResident(postsId, neighId) == null) {
            throw new WebApplicationException("Resource /residents/" + postsId + " does not exist.", 404);
        }
        ResidentProfileEntity e = residentLogic.associatePictureToResident(postsId, neighId, pic);

        ResidentProfileDetailDTO detailDTO = new ResidentProfileDetailDTO(e);

        LOGGER.log(Level.INFO, "Ended associating album to resident from resource: output: {0}", detailDTO);
        return detailDTO;
    }
